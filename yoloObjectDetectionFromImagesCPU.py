...
class EmotionDetector:
    ...
    def detect_emotions(self, frame):
        ...
        faces = self.face_cascade.detectMultiScale(gray, scaleFactor=1.3, minNeighbors=5, minSize=(50, 50))
...
class AgeDetector:
    def __init__(self):
        self.face_cascade = cv2.CascadeClassifier(cv2.data.haarcascades + 'haarcascade_frontalface_default.xml')
        self.age_history = {}  # Store prediction history for smoothing
        self.history_size = 5  # Number of past predictions to keep

    def update_history(self, face_id, age):
        """Update age history for temporal smoothing"""
        if face_id not in self.age_history:
            self.age_history[face_id] = []
        
        self.age_history[face_id].append(age)
        if len(self.age_history[face_id]) > self.history_size:
            self.age_history[face_id].pop(0)

    def get_smoothed_age(self, face_id):
        """Get temporally smoothed age"""
        if face_id not in self.age_history or not self.age_history[face_id]:
            return None
        
        # Return average age from history
        return int(sum(self.age_history[face_id]) / len(self.age_history[face_id]))
    ...
    def detect_age(self, frame):
        ...
        for (x, y, w, h) in faces:
            ...
            try:
                ...
                # Use face coordinates as a simple face ID for temporal smoothing
                face_id = f"{x}_{y}"
                self.update_history(face_id, age)
                smoothed_age = self.get_smoothed_age(face_id)
                
                if smoothed_age is not None:
                    # Position the age text below any existing text
                    cv2.putText(frame, f"Age: {smoothed_age}", (x, y + h + 20), 
...
if __name__ == "__main__":
    try:
        ...
        object_detector.set_gender_classifier(gender_classifier)
